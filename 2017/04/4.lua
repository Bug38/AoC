local input = [[sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp
...
--for i=1, #phrases do
--  local correct = true
--  for j=1, #phrases[i]-1 do
--    current = phrases[i][j]
--    for k = j+1, #phrases[i] do
--      tester = phrases[i][k]
...
--    end
--  end
--  if correct == true then count = count + 1 end
--end
--
--print(count)
...
for i=1, #phrases do
  ...
  for j=1, #phrases[i]-1 do
    ...
    for k = j+1, #phrases[i] do
      tester = phrases[i][k]
      if word_sort(current) == word_sort(tester) then
        correct = false
      end
    end
  end
